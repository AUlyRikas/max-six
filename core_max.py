...
# -*- coding: utf-8 -*-
"""
core_max.py —— MAX预测核心模块（作弊版，GIT网站展示）
==========================================================================
整合所有在极限扫描中被证实有效的信号：
  1. 遗漏值（权重2.0，阈值8/20分段）
  2. 金标投票梯度惩罚（[3, 8, 15, 30]）
  3. 冷却距离衰减（[10, 5, 2]）
  4. L3杀肖（权重5）
  5. 固定杀肖（权重15）
  6. 特码金标杀肖（权重10）
...
def output_text(result):
    lines = []
    lines.append(f"MAX预测（作弊版-GIT展示） | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("=" * 50)
    ...
    lines.append(f"基准命中率(作弊版极限): 九肖99.35% | 六肖95.84%")
    lines.append("-" * 30)
    lines.append(f"[信号源]")
